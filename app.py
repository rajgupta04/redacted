    ...
    <script>
        ...
        async function submitRedaction() {
            ...
            rows.forEach(row => {
                ...
                if (ignoreCheckboxes.includes(type)) return;
            ...
            try {
                // Step 1: Submit redaction job
                const submitRes = await fetch('/api/redact-custom', {
                    method: 'POST',
                ...
                if (!submitRes.ok) {
                    ...
                    throw new Error(err.detail || "Failed to submit redaction job.");
                }
                    ...
                    function pollRedaction() {
                        ...
                        .then(job => {
                            if (job.status === "completed") {
                                ...
                                reject(new Error(job.error || "Redaction failed on server."));
                            } else {
                                btn.innerHTML = `<div class="spinner" style="width: 16px; height: 16px; border-width: 2px;"></div> ${job.progress || "Processing..."}`;
                                setTimeout(pollRedaction, 1000);
                            }
                        })
                        .catch(reject);
                    }
                ...
                // Step 3: Download the redacted file
                btn.innerHTML = `<div class="spinner" style="width: 16px; height: 16px; border-width: 2px;"></div> Downloading...`;
